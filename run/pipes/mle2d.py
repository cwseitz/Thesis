...
class PipelineMLE2D:
    ...
    def localize(self,plot_spots=False,plot_fit=False,tmax=None,run_deconv=False,fit=True):
    ...
    def fit(self,frame,spots,plot_fit=False):
        ...
        for i in spots.index:
            ...
            opt = MLE2D_BFGS(theta0,adu,self.config)
            theta_mle, loglike, conv, err =\
            opt.optimize(max_iters=config['max_iters'],plot_fit=plot_fit)
            ...
            spots.at[i, 'x_err'] = err[0]
            spots.at[i, 'y_err'] = err[1]
            spots.at[i, 'N0_err'] = err[2]
