...
from DeepSMLM.torch.models import LocalizationVAE1
...
class VAE1_Test:
    ...
    def load_model(self):
        ...
        model = LocalizationVAE1(args['latent_dim'],args['nx'],args['ny'])
        model = model.to(device=device)
        checkpoint = self.modelpath+self.modelname+'/'+self.modelname+'.pth'
        checkpoint = torch.load(checkpoint, map_location=device)
        model.load_cnn(self.cnn_path,self.cnn_name)
