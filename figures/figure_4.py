...
import pandas as pd
from SMLM.utils import KDE, FRC
...
class Figure_4:
    ...
    def plot(self,niters=10):
        ...
        frc_curve_ = np.zeros((niters,len(fracs),1674)) #depends on KDE pixel size
        ...
        for m in range(niters):
            ...
            for n,frac in enumerate(fracs):
                ...
                frc = FRC(spots)
                freq, thres, frc_curve, res = frc.compute_frc(x_range,y_range,nsamples=nsamples,sigma=2.0,scale=100.0,plot_kde=True,plot_fft=True)
                frc_res_[m,n] = res
        ...
        ax[0].set_xlabel(r'Spatial frequency ($\mathrm{um}^{-1}$)')
        ax[0].set_ylabel('FRC')
        ax[0].legend()
        ...
        ax[1].set_ylabel(r'$q_{R}\;\mathrm{(um)}$',color='royalblue')
        ax2 = ax[1].twinx()
        ax2.plot(fracs,1/avg,color='red',marker='o')
        ax2.set_ylabel(r'R ($\mathrm{um}^{-1}$)',color='red')
        plt.tight_layout()
        plt.show()
