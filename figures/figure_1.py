...
class Figure_1:
    ...
    def plot(self,niters=10,pos=['x','y']):
        spots = self.spots
        ...
        frc_curve_ = np.zeros((niters,len(fracs),1100)) #depends on KDE pixel size

        nlocal = len(spots)
        for m in range(niters):
            x_range = (0,200); y_range = (0,200)
            spots = spots.loc[(spots[pos[0]] > x_range[0]) & (spots[pos[0]] < x_range[1])]
            spots = spots.loc[(spots[pos[1]] > y_range[0]) & (spots[pos[1]] < y_range[1])]
            for n,frac in enumerate(fracs):
                nsamples = int(round(frac*nlocal))
                print(f'Iteration {m}, Fraction: {frac}')
                frc = FRC(spots)
                freq, thres, frc_curve, res = frc.compute_frc(x_range,y_range,nsamples=nsamples,sigma=2.0,
                scale=100.0,plot_kde=False,plot_fft=False)
                frc_res_[m,n] = res
